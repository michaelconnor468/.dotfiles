(deftheme one-dark "Atom One Dark based theme")

(defconst one-dark-colors-alist
  (let* ((256color  (eq (display-color-cells (selected-frame)) 256))
         (colors `(("one-dark-accent"   . "#528BFF")
                   ...
                   ("one-dark-bg"       . (if ,256color "color-235" "#141414"))
...
(one-dark-with-color-variables
  (custom-theme-set-faces
   ...
   ;; tab-bar-mode
   `(tab-bar-tab-inactive ((t (:background ,one-dark-bg-hl :foreground ,one-dark-fg))))
   `(tab-bar-tab          ((t (:background ,one-dark-bg :foreground ,one-dark-purple))))
   `(tab-bar              ((t (:background ,one-dark-bg-hl))))
   ))
